...
class oneshot_t_test extends timer_t_test_base;

    // registration of component tools
    `uvm_component_utils( oneshot_t_test )

    uvm_sequence_base seq;
    // Constructor - creates new instance of this class
    function new( string name = "oneshot_t_test", uvm_component parent = null );
    ...
    // Configures ONE_SHOT mode and lets the timer run until it disables itself.
    task run_phase(uvm_phase phase);
        ...
        // 3. ONE SHOT mode
        mode_seq = timer_t_sequence_ONE_SHOT::type_id::create("one_shot_seq");
        mode_seq.start(m_env_h.m_timer_t_agent_h.m_sequencer_h);

        // 4. běh (delší než u autorestart)
